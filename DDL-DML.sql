...
COMMIT: as we have seen that all the DDL commands are auto commited by default but for other commands we explicitly 
        save the changes permanantly so, we can use COMMIT to Saves all changes made in the current transaction to the database.
